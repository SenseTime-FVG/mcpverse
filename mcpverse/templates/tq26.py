...
question = 'What is the train number with the shortest travel time from Shenzhen North(深圳北) to Liuzhou(柳州) the day after tomorrow, and how long does it take?'
...
def get_reference():
    ...
    get_tickets_node = [{"mcp": "12306-mcp", "id": "q2", "tool": "get-tickets", "args": {"date": add_days(get_time_res,2), "fromStation": "IOQ", "toStation": "LZZ"}}]
    ...
    if find:
        reference = f"On the day after Tomorrow, the train number with the shortest travel time from Shenzhen North(深圳北) to Liuzhou(柳州) is {train_no}, and it takes {cost_time}."
    else:
        reference = f"On the day after Tomorrow, there is no train from Shenzhen North(深圳北) to Liuzhou(柳州)."
    return reference
