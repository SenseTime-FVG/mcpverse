...
question = "What is the tasks included in Ross Girshick's latest paper according to the information on Paper with Code?"
def get_reference():
    

    node_1 = [{"mcp": "mcp-paperswithcode", "id": "q1", "tool": "list_papers_by_author_name", "args": {"author_name": "Ross Girshick","page": 1,"items_per_page": 20}}]
    ...
    if not tasks:
        return f"No tasks found for the latest paper {title_1} by Ross Girshick."
    else:
        return f"The tasks included in Ross Girshick's latest paper '{title_1}' are: {', '.join(tasks)}."
