...
class MCPPromptAgent(BaseAgent):
    ...
    def step(
        ...
        while True:
            ...
            generated_text=response.response.choices[0].message.content
            # breakpoint()
            flag, res = self._decode_model_res(generated_text)  # flag是False表示没有函数调用(对话终结)，True表示有函数调用
            if flag:
                try:
                    tool_call_requests = self._consturct_tool_call_request(res)
                except:
                    break
            else:
                break
            ...
            for tool_call_request in tool_call_requests:
                if (
                    ...
                    fc_res,tool_call_record = self._execute_tool(tool_call_request)
                    tool_call_records.append(tool_call_record)
                    
                    fc_results.append(fc_res)
            fc_results_str='<tool_call_response>'+str(fc_results)+'</tool_call_response>'
           
            fc_res_message = BaseMessage.make_user_message(
                role_name="User", content=fc_results_str
            )

            self.update_memory(response.output_messages[0],OpenAIBackendRole.ASSISTANT)
            self.update_memory(fc_res_message,OpenAIBackendRole.USER)
    ...
    def _decode_model_res(self,generated_text):
        ...
        try:
            reuslt = json.loads(json_str)
            return True, reuslt
        except:
            return False, generated_text
        
    def _consturct_tool_call_request(self,tool_call_list):
        ...
        for tool_call in tool_call_list:
            tool_call_request= ToolCallRequest(
                ...
                tool_call_id=uuid.uuid4().hex,  # Generate a unique ID for the tool call
            )
    ...
    async def _aget_model_response(
        ...
        try:
            ...
            response = await self.model_backend.arun(
                openai_messages, response_format, None
            )
    ...
    def _record_tool_calling(
        ...
        fc_res_str={func_name:result}
        return fc_res_str,tool_record
